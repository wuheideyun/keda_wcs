...
namespace KEDAClient
{
    ...
    public partial class KEDAForm : Form
    {
        ...
        //全局的列表数据变量
        List<FDispatchBackImf> dislist;
        List<DeviceBackImf> devsList;                    // = WcfMainHelper.GetDevList();
        List<FDispatchBackImf> tasksList;// = WcfMainHelper.GetDispatchList;
        //List<TaskRelationMember> definetasklist;// = WcfMainHelper.GetDefineTask();
        ...
        private void AlarmDataDisplay()
        {
            ...
            {
                if (item1.DevId != null && item1.DevType == "Magnet_Basic")
                {
                    int[] a = new int[] { 8, 10, 12, 13, 14, 15, 16, 20 };
                    for (int i = 0; i < a.Length; i++)
                    {
                        if (item1.ProtyList[a[i]].RValue == "1")
                        {
                            ListViewItem item = new ListViewItem(item1.DevId);  // AGV设备ID
                            item.SubItems.Add(item1.ProtyList[a[i]].SenType); // 类型名称
                            item.SubItems.Add(item1.ProtyList[a[i]].ChannelId.ToString());  // 通道编号
                            item.SubItems.Add(item1.ProtyList[a[i]].RValue);  // 实时值
                            item.SubItems.Add(item1.ProtyList[a[i]].EValue); // 描述值                                 
        ...
        private void OtherdevDisplay()
        {
            ...
            foreach (var item1 in devsList)
            {
                if (item1.DevType == "WK_PLC")
                {
                    int[] sens = new int[] { 0, 1, 2, 3 };
                    ListViewItem item = new ListViewItem(item1.DevId); // PLC的id
                    item.SubItems.Add(item1.DevType);  // PLC的型号
                    item.SubItems.Add(item1.IsAlive?"在线":"离线");  // PLC的状态
                    item.SubItems.Add(item1.ProtyList[sens[0]].RValue); // 货物状态
                    item.SubItems.Add(item1.ProtyList[sens[1]].RValue);// 电机状态
                    item.SubItems.Add(item1.ProtyList[sens[2]].RValue); // 故障状态
                    item.SubItems.Add(item1.ProtyList[sens[3]].RValue); // 备用信息
        ...
        private void VehiclesDisplay()
        {
            ...
            foreach (var item1 in devsList)
            {
                if (item1.DevType == "Magnet_Basic")
                {
                    // 状态 、运行方向、电量、充电状态
                    int[] sens = new int[] { 0,1, 4, 6, 7 };
                    ListViewItem item = new ListViewItem(item1.DevId); // 车辆编号
                    item.SubItems.Add(item1.DevType); // 车辆型号   
                    item.SubItems.Add(item1.IsAlive?"在线":"离线"); // 车辆状态    
                    item.SubItems.Add(item1.ProtyList[sens[0]].RValue); // 运行状态  
                    item.SubItems.Add(item1.ProtyList[sens[1]].RValue); // 地标   
                    item.SubItems.Add(item1.ProtyList[sens[3]].RValue); // 电量
                    item.SubItems.Add(item1.ProtyList[sens[4]].RValue); // 充电状态
        ...
        private void TaskInformDisplay()
        {
            ...
            {
                ListViewItem item = new ListViewItem(item1.Id); // 任务id
                item.SubItems.Add(item1.Content); // 任务信息
                item.SubItems.Add(item1.Statue.ToString()); // 任务状态
                item.SubItems.Add(item1.Source); // 触发源
                item.SubItems.Add(item1.Dev); // 任务设备
                item.SubItems.Add(item1.Path); // 任务路径
                item.SubItems.Add(item1.CtrType.ToString()); // 控制参数，枚举类型
                //item.SubItems.Add(item1.); // 指令信息
                item.SubItems.Add(item1.BackMsg); // 信息
                item.SubItems.Add(item1.StartTime); // 触发时间
                item.SubItems.Add(item1.EndTime); // 完成时间

                if (taskStatus.ContainsKey(item1.Id))
                {
                    taskStatus[item1.Id] = "startmissioin";
                }
                else
                {
                    taskStatus.Add(item1.Id, "startmission");
                }
        ...
        {
            List<FDispatchBackImf> dislist = WcfMainHelper.GetDispatchList();
            ...
            {
                ListViewItem item = new ListViewItem(item1.Id); // 调度id
                                                                     // item.SubItems.Add(item1.TaskImf); // 调度信息
                item.SubItems.Add(" 测试"); // 调度信息
                item.SubItems.Add(item1.Statue.ToString()); // 调度状态
                item.SubItems.Add(item1.Source); // 触发源
                item.SubItems.Add(item1.Dev); // 调度设备
                item.SubItems.Add(item1.Path); // 调度路径
                item.SubItems.Add(item1.CtrType.ToString()); // 控制参数，枚举类型             
        ...
        private void buttonSend_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                WcfMainHelper.SendOrder(vehicleslist.FocusedItem.Text, new FControlOrder(this.comboBox1.Text, order, Convert.ToInt32(textBox1.Text)));
        ...
        public void StopAGV()
        {
            WcfMainHelper.InitPara(_severIp, "", "");
            // 1是快速停止、0是慢速
            if (WcfMainHelper.SendOrder(vehicleslist.FocusedItem.Text, new FControlOrder("停止" + LocSite, 2, 0)))
            {
        ...
        {
            if (vehicleslist.FocusedItem == null)
            {
        ...
        private void agvForwordMove_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (WcfMainHelper.SendOrder(vehicleslist.FocusedItem.Text, new FControlOrder("前进启动" + LocSite, 1, 1)))
                {
                    //记录agv状态
        ...
        private void agvBackMove_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (WcfMainHelper.SendOrder(vehicleslist.FocusedItem.Text, new FControlOrder("后退" + LocSite, 1, 2)))
                {
                    //记录agv状态
        ...
        private void startmission_Click(object sender, EventArgs e)
        {
            ...
            else if (!pausemission.Enabled)
            {
                ...
                if (WcfMainHelper.CtrDispatch(taskInformlist.FocusedItem.Text, EnumCtrType.Start))
                {
                    SetOutputMsg("开始任务成功");
                }
            }
        ...
        {
            List<FDispatchBackImf> taskList = WcfMainHelper.GetDispatchList();
            ...
            if (tasksList != null && tasksList.Count != 0)
            {
                ...
                else
                {
                    ...
                    else if (MessageBox.Show("当前站点派发任务未完成，是否暂停？", "提示", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
                    {
                        ...
                        if (WcfMainHelper.CtrDispatch(taskInformlist.FocusedItem.Text, EnumCtrType.Pause))
                        {
                            SetOutputMsg("任务暂停成功！");
                        }
                    }
        ...
        private void endmission_Click(object sender, EventArgs e)
        {
            ...
            if (tasksList != null && tasksList.Count != 0)
            {
                ...
                else
                {
                    ...
                    if (WcfMainHelper.CtrDispatch(taskInformlist.FocusedItem.Text, EnumCtrType.Stop))
                    {
                        SetOutputMsg("结束任务成功");
                    }
                }
        ...
        {
            if (taskInformlist.FocusedItem == null)
            {
        ...
        { 
            ...
            {
                List<DeviceBackImf> devsList = WcfMainHelper.GetDevList();
                if (devsList == null) return;
                listv.BeginUpdate();
                for (int i = 0; i < devsList.Count; i++)
                {
                    if (devsList[i].DevType == "Magnet_Basic" && devsList[i].IsAlive)
                    {
                        if (listv.Name.Equals("vehicleslist"))
                        {
                            ...
                            {
                                listv.Items[i].SubItems[2].Text = dev.IsAlive.ToString();  // AGV在线状态      
                                                                          
                                // 判断AGV是停止还是运行，1为运行、3为停止
                                if (dev.ProtyList[4].RValue == "1")
                                {
                                    // 运行方向： 0 前进  1后退 
                                    if (dev.ProtyList[ConstSetBA.运行方向].RValue == "0")
                                    {
                                        listv.Items[i].SubItems[3].Text = "前进"; // 运行状态：前进
                                    }
                                    //  item1.ProtyList[sens[1]].RValue == "1"  后退
                                    else
                                    {
                                ...
                                listv.Items[i].SubItems[4].Text = dev.ProtyList[1].RValue;  // 地标
                                listv.Items[i].SubItems[5].Text = dev.ProtyList[6].RValue;  // 电量
                                // 充电状态            
                                if (dev.ProtyList[ConstSetBA.充电状态].RValue == "1")
                                {
                                    listv.Items[i].SubItems[6].Text="正充电";
                                }
                                else if (dev.ProtyList[ConstSetBA.充电状态].RValue == "2")
                                {
                                    listv.Items[i].SubItems[6].Text = "充电完成";
                                }
                                else if (dev.ProtyList[ConstSetBA.充电状态].RValue == "3")
                                {
                        ...
                        else if (listv.Name.Equals("alarmlist"))
                        {
                            ...
                            for (int j = 0; j < a.Length; j++)
                            {
                                ...
                                if (devsList[i].ProtyList[a[j]].RValue == "1")
                                {
                                    DeviceBackImf dev = devsList.Find(c => { return c.DevId == listv.Items[i].SubItems[0].Text; });

                                    listv.Items[i].SubItems[3].Text = dev.ProtyList[a[j]].RValue;
                                    //listv.Items[i].SubItems[3].Text = devsList[i].ProtyList[a[j]].RValue;
                                }
                            }
                        }
                    }
        ...
        private void RefreshtaskInform()
        {
            ...
            {
                ListViewItem item = new ListViewItem(tasksList[i].Id); // 任务id
                item.SubItems.Add(tasksList[i].Content); // 任务信息
                item.SubItems.Add(tasksList[i].Statue.ToString()); // 任务状态
                item.SubItems.Add(tasksList[i].Source); // 触发源
                item.SubItems.Add(tasksList[i].Dev); // 任务设备
                item.SubItems.Add(tasksList[i].Path); // 任务路径
                item.SubItems.Add(tasksList[i].CtrType.ToString()); // 控制参数，枚举类型
                item.SubItems.Add(tasksList[i].OrderInfo); // 指令信息
                item.SubItems.Add(tasksList[i].BackMsg); // 信息
                item.SubItems.Add(tasksList[i].StartTime); // 触发时间

                if (taskStatus.ContainsKey(tasksList[i].Id))
                {
                    taskStatus[tasksList[i].Id] = "startmissioin";
                }
                else
                {
                    taskStatus.Add(tasksList[i].Id, "startmission");
                }
                // 显示项
                taskInformlist.Items.Add(item);
            }
        ...
        private void RefreshdispatchInform()
        {
            ...
            {
                ListViewItem item = new ListViewItem(item1.Id); // 调度id
                item.SubItems.Add(item1.Content); // 调度信息
                item.SubItems.Add(item1.Statue.ToString()); // 调度状态
                item.SubItems.Add(item1.Source); // 触发源
                item.SubItems.Add(item1.Dev); // 调度设备
                item.SubItems.Add(item1.Path); // 调度路径
        ...
        {
            //List<TaskRelationMember> memberlist = WcfMainHelper.GetDefineTask();
            try
            {
                //if (definetasklist != null)
                //{
        ...
        {
            //List<TaskRelationMember> definetasklist = WcfMainHelper.GetDefineTask();
            ////List<GfxServiceContractTaskExcute.FDispatchBackImf> taskList = WcfMainHelper.GetDispatchList;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            List<FDispatchBackImf> result = WcfMainHelper.GetDispatchList();

            if (result != null)
            {
                foreach (var item in result)
                {
                    EnumResultType s = WcfMainHelper.GetDipatchStatue(item.Id);

                    //WcfMainHelper.CtrDispatch(item.DisGuid, EnumCtrType.Stop);
                    WcfMainHelper.CtrDispatch(item.Id, EnumCtrType.Stop);
                }
        ...
        private void queryDataThreadFunc()
        {
            while (true)
            {
                ...
                else
                {
                    ...
                    devsList = WcfMainHelper.GetDevList();
                    devOk = true;
                    switch (tabControl1SelectIndex)
                    {
                        case 0://0 task
                            //tasksList = WcfMainHelper.GetDispatchList;
                            //definetasklist = WcfMainHelper.GetDefineTask();
                            taskOk = true;
                            break;
                        case 1://1 alarms
        ...
        private void LoadStaHasGoods()
        {
            while (true)
            {
                ...
                if (unloadState == 0)
                {
                    ...
                    {
                        DeviceBackImf dev = devsList.Find(c => { return c.DevType == "PLC" && c.IsAlive && c.ProtyList[0].RValue == "1" && c.DevId == "PLC0000001"; });
        ...
        private void HasFreeAGV()
        {
            while (true)
            {
                ...
                if (_loadStaHasGoods && unloadState == 1)
                {
                    ...
                    if (devsList != null && devsList.Count != 0)
                    {
                        DeviceBackImf dev = devsList.Find(c => { return c.DevType == "Magnet_Basic" && c.IsAlive && c.ProtyList[1].RValue == _loadWaitSta && c.ProtyList[9].RValue == "true" && c.ProtyList[7].RValue == "0"; });
        ...
        private void SendAGVtoLoadSta()
        {
            while (true)
            {
                ...
                if (_HasFreeAGV && unloadState == 2)
                {
                    ...
                    dis.Content = "AGV去窑尾接货";

                    dis.NavigationType = EnumNavigationType.Magnet;

                    dis.Id = "AGV从当前等待位到窑尾装载点";
        ...
        private void LoadStaHasAGV()
        {
            while (true)
            {
                ...
                {
                    DeviceBackImf dev = devsList.Find(c => { return c.DevType == "Magnet_Basic" && c.ProtyList[1].RValue == _loadgoodsSta; });
        ...
        private void LoadLineRollerTable()
        {
            while (true)
            {
                ...
                if (_loadStaHasAGV && unloadState == 4)
                {
                    if (devsList != null && devsList.Count != 0)
                    {
                        DeviceBackImf dev = devsList.Find(c => { return c.DevId == "PLC0000001" && c.IsAlive; });
                        // 装载点有AGV，线边辊台准备出料
                        if (dev != null)
                        {
                            // 线边辊台下料正转  Order1: 2,1
                            WcfMainHelper.SendOrder(dev.DevId, new FControlOrder("线边辊台下料 正转" + LocSite, 1, 2, 1));
                            unloadState = 5;
                        }
        ...
        private void AGVHasGoods()
        {
            while (true)
            {
                ...
                if (unloadState == 5)
                {
                    if (devsList != null && devsList.Count != 0)
                    {
                        DeviceBackImf dev = devsList.Find(c => { return c.ProtyList[1].RValue == _loadgoodsSta && c.ProtyList[35].RValue == "1" && c.DevType == "Magnet_Basic"; });
            ...
            while (true)
            {
                ...
                if (_AGVHasGoods && unloadState == 6)
                {
                    if (devsList != null && devsList.Count != 0)
                    {
                        ...
                        {
                            // 线边辊台上料正转  Order1: 1,3
                            WcfMainHelper.SendOrder(dev.DevId, new FControlOrder("线边辊台下料 停止" + LocSite, 1, 2, 3));
        ...
        private void SendAGVtoUnloadWaitSta()
        {
            while (true)
            {
                ...
                {
                    //任务中一个调度节点
                    FDispatchOrder dis = new FDispatchOrder(); dis.NavigationType = EnumNavigationType.Magnet;

                    dis.Content = "AGV从窑尾装载点到窑头卸载等待区";
        ...
        private void UnloadWaitStaHasAGV()
        {
            while (true)
            {
                ...
                {
                    DeviceBackImf dev = devsList.Find(c => { return c.DevType == "Magnet_Basic" && c.ProtyList[1].RValue == _unloadWaitSta && c.ProtyList[35].RValue == "1"; });
        ...
        private void UnloadStaHasGoods()
        {
            while (true)
            {
                ...
                if (_HasUnloadAGV && loadState == 1)
                {
                    ...
                    if (devsList != null && devsList.Count != 0)
                    {
                        DeviceBackImf dev = devsList.Find(c => { return c.DevType == "PLC" && c.IsAlive && c.ProtyList[0].RValue == "1" && c.DevId == "PLC0000002"; });
                        {
                            // PLC 在线，且货物状态为1
                            if (dev != null)
                            {
        ...
        private void UnloadStaHasAGV()
        {
            while (true)
            {
                ...
                {
                    DeviceBackImf dev = devsList.Find(c => { return c.DevType == "Magnet_Basic" && c.ProtyList[1].RValue == _unloadgoodsSta && c.ProtyList[35].RValue == "1"; });
        ...
        private void UnloadLineRollerTable()
        {
            while (true)
            {
                ...
                if (_unloadStaHasAGV && loadState == 4)
                {
                    if (devsList != null && devsList.Count != 0)
                    {
                        DeviceBackImf dev = devsList.Find(c => { return c.DevType == "PLC" && c.IsAlive && c.DevId == "PLC0000002"; });

                        // 装载点有有货的AGV，线边辊台准备接货
                        if (dev != null)
                        {
                            // 线边辊台上料 正转  Order1: 1,1
                            WcfMainHelper.SendOrder(dev.DevId, new FControlOrder("线边辊台上料 停止" + LocSite, 1, 1, 1));

                            Thread.Sleep(3000);
                            // 判断线边辊台电机是否正在转动
                            if (dev.ProtyList[1].RValue == "1")
                            {
                                _isRorate = true;
                                loadState =5;
        ...
        private void ReadyToUnload()
        {
            while (true)
            {
                ...
                if (_isRorate && loadState == 5)
                {
                    if (devsList != null && devsList.Count != 0)
                    {
                        DeviceBackImf dev = devsList.Find(c => { return c.DevType == "Magnet_Basic" && c.IsAlive && c.ProtyList[1].RValue == _unloadgoodsSta && c.ProtyList[9].RValue == "true" && c.ProtyList[35].RValue == "1"; });

                        // 启动窑头卸载点的AGV 车载辊台转动卸货
                        if (dev != null)
                        {
                            WcfMainHelper.SendOrder(dev.DevId, new FControlOrder("车载辊台下料 正转" + LocSite, 1, 2, 1));
                            loadState = 6;
        ...
        private void AGVGoodsStatue()
        {
            while (true)
            {
                ...
                {
                    DeviceBackImf dev = devsList.Find(c => { return c.DevType == "Magnet_Basic" && c.ProtyList[35].RValue == "0"; });
        ...
        private void UnloadLineRollerTableStop()
        {
            while (true)
            {
                ...
                if (!_AGVGoodsStatue && loadState == 7)
                {
                    if (devsList != null && devsList.Count != 0)
                    {
                        DeviceBackImf dev = devsList.Find(c => { return c.DevType == "PLC" && c.DevId == "PLC0000002" && c.IsAlive; });

                        // 装载点有AGV，线边辊台准备出料
                        if (dev != null)
                        {
                            // 线边辊台上料  停止  Order1: 1,3
                            WcfMainHelper.SendOrder(dev.DevId, new FControlOrder("线边辊台上料 停止" + LocSite, 1, 1, 3));
