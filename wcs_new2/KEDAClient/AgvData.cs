    ...
    public class AgvData : IAgvData
    {
        ...
        public AgvData(DeviceBackImf dev)
        {
            try
            {
                ...
                IsOnLine = Convert.ToBoolean(dev.IsAlive);

                if (!IsOnLine) { return; }

                //devNum = string.Format("{0}{1}", dev.DevId.Substring(0, 2), devNum);

                var sensor = dev.IGet("0001");

                //var sensor = dev.ProtyList.Find(c => { return c.SenId == string.Format("{0}0001", devNum); });

                if (null != sensor) { AgvState = Convert.ToByte(sensor.RValue); }

                sensor = dev.IGet("0002");

                if (null != sensor) { DbNum = Convert.ToInt32(sensor.RValue); }

                sensor = dev.ProtyList.Find(c => { return c.SenId == string.Format("{0}0003", dev.DevId); });
