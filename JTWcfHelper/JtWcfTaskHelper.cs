...
namespace JTWcfHelper
{
    ...
    public class JtWcfTaskHelper
    {
        ...
        /// <summary>
        /// 开启任务
        /// </summary>客户端通过调用此方法触发服务端执行指定id的任务
        /// <returns></returns>
        public static bool StartTask(string taskId,string markMsg)
        {
        ...
        /// <summary>
        /// 控制指定任务状态
        /// DisOrderCtrTypeEnum ctr 控制参数：Start = 0, Pause = 1,Stop = 2 
        /// </summary>
        /// <param name="devid">设备id</param>
        /// <returns></returns>
        public static bool CtrTask(string taskId, DisOrderCtrTypeEnum ctr, string markMsg)
        {
            if (!_channelState && !Open()) { return false; }

            lock (_ans) { try { return _backupsClass.CtrTask(taskId, ctr, markMsg); } catch { return false; } }
        }
        ...
        /// </summary>
        /// <param name="devid">设备id</param>
        /// <returns>DisOrderCtrTypeEnum ctr 控制参数：Start = 0, Pause = 1,Stop = 2 </returns>

        public static bool AdminCtrTask(string disGuid, DisOrderCtrTypeEnum ctr)
        {
            if (!_channelState && !Open()) { return false; }

            lock (_ans) { try { return _backupsClass.AdminCtrTask(disGuid, ctr); } catch { return false; } }
        }
        ...
        /// <returns></returns>
        public static TaskBackImf GetTask(string taskid,string markMsg)
        {
            if (!_channelState && !Open()) { return null; }

            lock (_ans) { try { return _backupsClass.GetTask(taskid, markMsg); } catch { return null; } }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static GfxList<TaskBackImf> GetAllTask()
        {
            if (!_channelState && !Open()) { return null; }

            lock (_ans) { try { return _backupsClass.GetAllTask(); } catch { return null; } }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static GfxList<TaskRelationMember> GetDefineTask()
        {
            if (!_channelState && !Open()) { return null; }

            lock (_ans) { try { return _backupsClass.GetDefineTask(); } catch { return null; } }
        }
        ...
        /// <returns></returns>
        public static bool StartTaskTemp(string soure, OnceTaskMember task)
        {
            if (!_channelState && !Open()) { return false; }

            lock (_ans) { try { return _backupsClass.StartTaskTemp(soure, task); } catch { return false; } }
        }

        /// <summary>
        /// 获取指定 id 执行的任务信息
        /// </summary>
        /// <param name="disGuid"></param>
        /// <returns></returns>
        public static TaskBackImf GetExcTask(string disGuid)
        {
            if (!_channelState && !Open()) { return null; }

            lock (_ans) { try { return _backupsClass.GetExcTask(disGuid); } catch { return null; } }
        }
        #endregion
    }
}
